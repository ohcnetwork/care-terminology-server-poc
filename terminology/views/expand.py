...
def filter_and_paginate_concepts(concept_ids, filter_text, display_language, 
                                include_designations, count, offset):
    """
    Filter concepts by text using matching logic
    ...
    try:
        ...
        for i in range(0, len(concept_ids), batch_size):
            ...
            query = build_text_query(batch, normalized_filter, display_language)
...
def build_text_query(concept_ids, filter_text, language):
    return {
        "query": {
            "bool": {
                "must": [
...
def get_total_matching_count(concept_ids, filter_text, display_language):
    ...
    """
    try:
        query = build_text_query(concept_ids, filter_text, display_language)
